...
7. `users.db`: User login (hash scrypt, secret TOTP terenkripsi, recovery codes) **dan** tabel `sessions` panel — dimiliki `panel/server/auth.js` & `manager/permission_manager`.
8. `platform.db`: Meta config, `schema_migrations`, runner state 24/7, storage stats.
9. `locks.db`: `lock_registry` + `lock_events` (metadata file-lock lintas proses).

> **Koreksi dari kode aktual**: TIDAK ada `meta.db` maupun `vault.db` — metadata migrasi hidup di tabel `meta`/`schema_migrations` DALAM tiap DB, dan vault rahasia disimpan terenkripsi di `secrets/vault.enc` (payload JSON, `lib/vault.js`) + `secrets/configs/`. File `data/panel.db`, `data/sessions.db`, `data/vpanel.db` adalah artefak lama tanpa referensi `openDatabase` di kode aktif — jangan dipakai/ditulis.
...
npm test
```
- Menjalankan 44 berkas test (74 suite, 524 test) di `tests/unit/*.test.js` menggunakan test runner native `node:test`.
...
- `panel/templates/*.html` — Seluruh template halaman panel (14 template HTML: 12 halaman + login + error).
- `panel/static/panel.css` — CSS master Obsidian Dark Luxe.
- `manager/index.js` — Core orchestrator & REST API service manager.
- `panel/server/assistant.js` — Hermes AI Assistant reasoning engine & dynamic tools (routes `/api/assistant/*` dilayani PANEL di :8080, bridge ke Manager via ManagerClient).
