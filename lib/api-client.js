...
export class ManagerClient {
   ...
   */
  async request(method, path, { query = undefined, body = undefined, timeoutMs = undefined } = {}) {
    ...
    const timeout = Number.isInteger(timeoutMs) && timeoutMs > 0 ? timeoutMs : this.#timeoutMs;
    let res;
    let lastErr = null;
    // A2#14: retry HANYA untuk method idempoten (GET/HEAD). POST/PUT/PATCH/
    // DELETE satu percobaan — men-deploy ulang aksi non-idempoten karena
    // retry socket bisa membuat efek ganda (project ter-create dua kali).
      ...
      try {
        ...
        if (name === 'TimeoutError' || name === 'AbortError') {
          throw new VmPanelError(
            TIMEOUT,
            `manager request timed out after ${timeout}ms (${method} ${path})`,
            { method, path, timeoutMs: timeout },
          );
        }
        // Connection failed (e.g. stale keepalive socket hangup) — idempoten
        // saja yang boleh dicoba ulang sekali.
    ...
    if (lastErr) {
      throw new VmPanelError(
        UNREACHABLE,
        `cannot reach manager at ${url.origin} (${lastErr && lastErr.message ? lastErr.message : String(lastErr)})`,
        { method, path, reason: lastErr && lastErr.message ? lastErr.message : String(lastErr) },
      );
    }
